...
- NEXT after the redesign: the human redeploys on Vercel so the new UI is live, then the FRONTEND TestSprite tests are re-authored against the new DOM. The booking flow changed (select then Continue to review; new panel test ids like `hold-continue-btn`, `signin-cta`), so the old frontend plans under `tests/testsprite/frontend/` need updating before rerunning them.
